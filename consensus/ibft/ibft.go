...
func (i *backendIBFT) WhiteList(header *types.Header) []types.Address {
	i.logger.Debug("get white list", "number", header.Number)
	var whiteList []types.Address
	if header.Number == 0 {
		return whiteList
	}
	...
	for {
		parentSnap = i.getSnapshot(header.Number - 1)
		if parentSnap != nil {
			break
		}
		i.logger.Warn("get snapshot waiting ", "number", header.Number-1)
		time.Sleep(100 * time.Millisecond)
	}

	for _, address := range parentSnap.WhiteSet {
		var add types.Address
		copy(add[:], address[:])
		whiteList = append(whiteList, add)
	}
	return whiteList
}
