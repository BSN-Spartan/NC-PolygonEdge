...
func putIbftExtraValidators(h *types.Header, validators []types.Address, whiteAccountValidators []types.Address) {
	...
	ibftExtra := &IstanbulExtra{
		...
		WhiteAccountValidators: whiteAccountValidators,
		ProposerSeal:           []byte{},
		CommittedSeal:          [][]byte{},
	}
...
func (i *IstanbulExtra) MarshalRLPWith(ar *fastrlp.Arena) *fastrlp.Value {
	...
	if len(i.WhiteAccountValidators) == 0 {
		vv.Set(ar.NewNullArray())
	} else {
		// WhiteValidators
		whitevals := ar.NewArray()
		for _, a := range i.WhiteAccountValidators {
			whitevals.Set(ar.NewBytes(a.Bytes()))
		}
...
func (i *IstanbulExtra) UnmarshalRLPFrom(p *fastrlp.Parser, v *fastrlp.Value) error {
	...
	// whitelist
	{
		...
		if err != nil {
			return fmt.Errorf("list expected for whitelist")
		}

		if vals != nil {
			i.WhiteAccountValidators = make([]types.Address, len(vals))
			for indx, val := range vals {
				if err = val.GetAddr(i.WhiteAccountValidators[indx][:]); err != nil {
					return err
				}
			}
		}
