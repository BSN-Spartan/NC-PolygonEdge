...
type operator struct {
	...
	whiteAccountCandidates []*proto.Candidate
...
func (o *operator) getNextCandidate(snap *Snapshot) *proto.Candidate {
	r := rand.Intn(100)
	if r%2 == 0 {
		o.ibft.logger.Debug("operator getNextCandidate Consensus")
		return o.getNextConsensusCandidate(snap)
	}

	o.ibft.logger.Debug("operator getNextCandidate White Account")
	return o.getNextWhiteAccountCandidate(snap)
}

// getNextConsensusCandidate returns a candidate from the snapshot
func (o *operator) getNextConsensusCandidate(snap *Snapshot) *proto.Candidate {
...
// getNextWhiteAccountCandidate returns a candidate from the snapshot
func (o *operator) getNextWhiteAccountCandidate(snap *Snapshot) *proto.Candidate {
	o.candidatesLock.Lock()
	defer o.candidatesLock.Unlock()

	// first, we need to remove any candidates that have already been
	// selected as validators
	for i := 0; i < len(o.whiteAccountCandidates); i++ {
		addr := types.StringToAddress(o.whiteAccountCandidates[i].Address)

		// Define the delete callback method
		deleteFn := func() {
			o.whiteAccountCandidates = append(o.whiteAccountCandidates[:i], o.whiteAccountCandidates[i+1:]...)
			i--
		}

		// Check if the candidate is already in the validator set, and wants to be added
		if o.whiteAccountCandidates[i].Auth && snap.WhiteSet.Includes(addr) {
			deleteFn()
	...
	// now pick the first candidate that has not received a vote yet
	for _, c := range o.whiteAccountCandidates {
		addr := types.StringToAddress(c.Address)

		count := snap.WhiteCount(func(v *Vote) bool {
			return v.Address == addr && v.Validator == o.ibft.validatorKeyAddr
		})
...
func (o *operator) Propose(ctx context.Context, req *proto.Candidate) (*empty.Empty, error) {
	o.ibft.logger.Debug("operator propose", "address", req.Address, "auth", req.Auth, "mode", req.Mode.String())

	if req.Mode == proto.CandidateType_whiteAccount {
		return o.whiteAccountPropose(req)
	}

	return o.consensusPropose(req)
}

// Propose proposes a new consensus candidate to be added / removed from the consensus validator set
func (o *operator) consensusPropose(req *proto.Candidate) (*empty.Empty, error) {
...
// Propose proposes a new white account list candidate to be added / removed from the white account list validator set
func (o *operator) whiteAccountPropose(req *proto.Candidate) (*empty.Empty, error) {
	var addr types.Address
	if err := addr.UnmarshalText([]byte(req.Address)); err != nil {
		return nil, err
	}

	o.ibft.logger.Debug("operator whiteAccountPropose ", "address", addr.String())
	// check if the candidate is already there
	o.candidatesLock.Lock()
	defer o.candidatesLock.Unlock()
	...
	// safe checks
	if req.Auth {
		if snap.WhiteSet.Includes(addr) {
			return nil, fmt.Errorf("the candidate is already a validator")
		}
	}
	...
	o.whiteAccountCandidates = append(o.whiteAccountCandidates, req)

	for _, candidate := range o.whiteAccountCandidates {
		o.ibft.logger.Debug("operator whiteAccountPropose whiteAccountCandidates ", "address", candidate.Address, "auth", candidate.Auth)
	}
...
func (o *operator) Candidates(ctx context.Context, req *empty.Empty) (*proto.CandidatesResp, error) {
	...
	resp := &proto.CandidatesResp{
		Candidates:      []*proto.Candidate{},
		WhiteCandidates: []*proto.Candidate{},
	}

	resp.Candidates = append(resp.Candidates, o.candidates...)

	resp.WhiteCandidates = append(resp.WhiteCandidates, o.whiteAccountCandidates...)
	for _, candidate := range resp.WhiteCandidates {
		o.ibft.logger.Debug("operator Candidates whiteAccountCandidates ", "address", candidate.Address, "auth", candidate.Auth)
