...
var (
	...
	// Magic nonce number to vote on adding a new validator
	nonceWhiteAccountAuthVote = types.Nonce{0x00, 0x00, 0x00, 0x01, 0xff, 0xff, 0xff, 0xff}

	// Magic nonce number to vote on removing a validator.
	nonceWhiteAccountDropVote = types.Nonce{0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00}
)
...
func (poa *PoAMechanism) verifyHeadersHook(nonceParam interface{}) error {
	...
	if nonce != nonceDropVote && nonce != nonceAuthVote && nonce != nonceWhiteAccountDropVote && nonce != nonceWhiteAccountAuthVote {
...
func (poa *PoAMechanism) processHeadersHook(hookParam interface{}) error {
	...
	var authorize bool
	var candidateType proto.CandidateType

	switch params.header.Nonce {
	case nonceAuthVote:
		authorize = true
		candidateType = proto.CandidateType_consensus
	case nonceDropVote:
		authorize = false
		candidateType = proto.CandidateType_consensus
	case nonceWhiteAccountAuthVote:
		authorize = true
		candidateType = proto.CandidateType_whiteAccount
	case nonceWhiteAccountDropVote:
		authorize = false
		candidateType = proto.CandidateType_whiteAccount
	default:
	...
	if candidateType == proto.CandidateType_consensus {
		return poa.processConsensus(params, authorize)
	} else {
		return poa.processWhiteAccount(params, authorize)
	}
}

func (poa *PoAMechanism) processConsensus(params *processHeadersHookParams, authorize bool) error {
	// validate the vote
	if authorize {
...
func (poa *PoAMechanism) processWhiteAccount(params *processHeadersHookParams, authorize bool) error {
	poa.ibft.logger.Debug("poa processWhiteAccount ", "miner", params.header.Miner.String(), "auth", authorize, "params", params)
	// validate the vote
	if authorize {
		// we can only authorize if they are not on the validators list
		if params.snap.WhiteSet.Includes(params.header.Miner) {
			return nil
		}
	} else {
		// we can only remove if they are part of the validators list
	...
	voteCount := params.snap.WhiteCount(func(v *Vote) bool {
		return v.Validator == params.proposer && v.Address == params.header.Miner
	})

	poa.ibft.logger.Debug("poa processWhiteAccount voteCount ", "proposer", params.proposer, "miner", params.header.Miner.String(), "voteCount", voteCount)
	if voteCount > 1 {
		// there can only be one vote per validator per address
		return fmt.Errorf("more than one proposal per validator per address found")
	}
	...
	if voteCount == 0 {
		// cast the new vote since there is no one yet
		params.snap.WhiteVotes = append(params.snap.WhiteVotes, &Vote{
			Validator: params.proposer,
			Address:   params.header.Miner,
			Authorize: authorize,
		})
	...
	// check the tally for the proposed validator
	tally := params.snap.WhiteCount(func(v *Vote) bool {
		return v.Address == params.header.Miner
	})
	poa.ibft.logger.Debug("poa processWhiteAccount tally ", "number", params.header.Number, "miner", params.header.Miner.String(), "tally", tally)
	// If more than a half of all validators voted
	if tally > params.snap.Set.Len()/2 {
		poa.ibft.logger.Debug("poa processWhiteAccount tally result", "miner", params.header.Miner.String(), "result", true, "authorize", authorize)
		if authorize {
			// add the candidate to the validators list
			params.snap.WhiteSet.Add(params.header.Miner)
		} else {
			// remove the candidate from the validators list
			params.snap.WhiteSet.Del(params.header.Miner)

			// remove any votes casted by the removed validator
			//params.snap.RemoveWhiteVotes(func(v *Vote) bool {
			//	return v.Validator == params.header.Miner
			//})
		}
...
func (poa *PoAMechanism) candidateVoteHook(hookParams interface{}) error {
	...
	if candidate := poa.ibft.operator.getNextCandidate(params.snap); candidate != nil {
		poa.ibft.logger.Debug("poa candidateVoteHook candidate ", "address", candidate.Address, "auth", candidate.Auth, "mode", candidate.Mode)

		params.header.Miner = types.StringToAddress(candidate.Address)

		if candidate.Mode == proto.CandidateType_consensus {
			if candidate.Auth {
			...
			if candidate.Auth {
				params.header.Nonce = nonceWhiteAccountAuthVote
			} else {
				params.header.Nonce = nonceWhiteAccountDropVote
			}
		}
	}
