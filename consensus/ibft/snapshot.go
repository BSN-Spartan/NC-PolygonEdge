...
func (i *backendIBFT) addHeaderSnap(header *types.Header) error {
	...
	snap := &Snapshot{
		...
		WhiteSet: extra.WhiteAccountValidators,
	}
...
func (i *backendIBFT) processHeaders(headers []*types.Header) error {
	...
	saveSnap := func(h *types.Header) {
		...
		i.logger.Debug("ibft save snap", "number", h.Number, "whiteAccts", snap.WhiteSet.Len(), "validators", snap.Set.Len())
	...
	for _, h := range headers {
		...
		i.logger.Debug("ibft equal snap ", "number", h.Number, "whiteAccts", snap.WhiteSet.Len(), "validators", snap.Set.Len())
...
func (s *Snapshot) Equal(ss *Snapshot) bool {
	...
	if len(s.WhiteVotes) != len(ss.WhiteVotes) {
		return false
	}
	...
	for indx := range s.WhiteVotes {
		if !s.WhiteVotes[indx].Equal(ss.WhiteVotes[indx]) {
			return false
		}
	}

	return s.Set.Equal(&ss.Set) && s.WhiteSet.Equal(&ss.WhiteSet)
}
...
// WhiteCount returns the vote tally.
// The count increases if the callback function returns true
func (s *Snapshot) WhiteCount(h func(v *Vote) bool) (count int) {
	for _, v := range s.WhiteVotes {
		if h(v) {
			count++
		}
	}
...
// RemoveWhiteVotes removes votes from the snapshot, based on the passed in callback
func (s *Snapshot) RemoveWhiteVotes(h func(v *Vote) bool) {
	for i := 0; i < len(s.WhiteVotes); i++ {
		if h(s.WhiteVotes[i]) {
			s.WhiteVotes = append(s.WhiteVotes[:i], s.WhiteVotes[i+1:]...)
			i--
		}
	}
}
...
func (s *Snapshot) Copy() *Snapshot {
	...
	ss := &Snapshot{
		...
		Set:        ValidatorSet{},
		WhiteVotes: make([]*Vote, len(s.WhiteVotes)),
		WhiteSet:   ValidatorSet{},
	}
	...
	for indx, vote := range s.WhiteVotes {
		ss.WhiteVotes[indx] = vote.Copy()
	}

	ss.Set = append(ss.Set, s.Set...)

	ss.WhiteSet = append(ss.WhiteSet, s.WhiteSet...)
...
func (s *Snapshot) ToProto() *proto.Snapshot {
	resp := &proto.Snapshot{
		...
		WhiteValidators: []*proto.Snapshot_Validator{},
		WhiteVotes:      []*proto.Snapshot_Vote{},
		Number:          s.Number,
		Hash:            s.Hash,
	}
	...
	// add white votes
	for _, vote := range s.WhiteVotes {
		resp.WhiteVotes = append(resp.WhiteVotes, &proto.Snapshot_Vote{
			Validator: vote.Validator.String(),
			Proposed:  vote.Address.String(),
			Auth:      vote.Authorize,
		})
	...
	// add addresses
	for _, val := range s.WhiteSet {
		resp.WhiteValidators = append(resp.WhiteValidators, &proto.Snapshot_Validator{
			Address: val.String(),
		})
	}
