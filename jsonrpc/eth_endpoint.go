...
type ethTxPoolStore interface {
	...
	TxSender(tx *types.Transaction) (types.Address, error)
...
type ethStateStore interface {
	...
	GetWhitelistOption() state.GetWhitelistOption
}
...
var (
	ErrProhibitedTransfer = errors.New("transfer prohibited for address")
...
func (e *Eth) GetBlockByNumber(number BlockNumber, fullTx bool) (interface{}, error) {
	...
	return e.toBlock(block, fullTx), nil
}
...
func (e *Eth) GetTransactionByHash(hash types.Hash) (interface{}, error) {
	...
	findSealedTx := func() *transaction {
		...
		for idx, txn := range block.Transactions {
			if txn.Hash == hash {
				...
				sender, err := e.store.TxSender(txn)
				if err == nil {
					tran.From = sender
				}
	...
	findPendingTx := func() *transaction {
		...
		if pendingTx, pendingFound := e.store.GetPendingTx(hash); pendingFound {
			tran := toPendingTransaction(pendingTx)
			sender, err := e.store.TxSender(pendingTx)
			if err == nil {
				tran.From = sender
			}
			return tran
...
func (e *Eth) EstimateGas(arg *txnArgs, rawNum *BlockNumber) (interface{}, error) {
	...
	if transaction.From != types.ZeroAddress {
		...
		if transaction.Value != nil && transaction.Value.Sign() > 0 {
			whiteAccounts := e.store.GetWhitelistOption()(header)
			e.logger.Debug("ETH EstimateGas white accounts", "number", header.Number, "whiteAccounts", len(whiteAccounts))
			isWhiteAccount := func(address *types.Address) bool {
				if address == nil {
					return false
				}
				for _, wacc := range whiteAccounts {
					if wacc == *address {
						return true
					}
				}
			...
			if !isWhiteAccount(&transaction.From) && !isWhiteAccount(transaction.To) {
				e.logger.Warn("transfer prohibited for address", "from", transaction.From, "to", transaction.To)
				return 0, ErrProhibitedTransfer
			}
