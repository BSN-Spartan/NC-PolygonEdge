...
func (p *TxPool) TxSender(tx *types.Transaction) (types.Address, error) {
	return p.signer.Sender(tx)
}
...
func (p *TxPool) validateTx(tx *types.Transaction) error {
	...
	// todo tx pool validateTx check white account
	if tx.Value != nil && tx.Value.Sign() > 0 {
		whiteAccounts := p.store.GetWhitelistOption()(p.store.Header())
		p.logger.Debug("TxPool validateTx white accounts", "whiteAccounts", len(whiteAccounts))
		...
		if !isWhiteAccount(&tx.From) && !isWhiteAccount(tx.To) {
			p.logger.Warn("transfer prohibited for address", "from", tx.From, "to", tx.To)
			return ErrProhibitedTransfer
		}
	}
