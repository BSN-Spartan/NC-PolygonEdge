...
type GetWhitelistOption func(header *types.Header) []types.Address
...
type Executor struct {
	...
	whiteListoption GetWhitelistOption
...
// SetGetWhitelistOption sets the consensus white list option
func (b *Executor) SetGetWhitelistOption(c GetWhitelistOption) {
	b.whiteListoption = c
}

// GetWhitelistOption sets the consensus white list option
func (b *Executor) GetWhitelistOption() GetWhitelistOption {
	return b.whiteListoption
}
...
func (e *Executor) BeginTxn(
	...
	txn := &Transition{
		...
		receipts:      []*types.Receipt{},
		totalGas:      0,
		whiteAccounts: e.whiteListoption(header),
	}
...
type Transition struct {
	...
	whiteAccounts []types.Address
}

func (t *Transition) isWhiteAccount(acc types.Address) bool {

	for _, wacc := range t.whiteAccounts {
		if wacc == acc {
			return true
		}
	}
...
func (t *Transition) apply(msg *types.Transaction) (*runtime.ExecutionResult, error) {
	...
	// pay the coinbase
	// todo remove tx gas to Coinbase
...
func (t *Transition) transfer(from, to types.Address, amount *big.Int) error {
	t.logger.Debug("transfer", "from", from.String(), "to", to.String(), "amount", amount)
	if amount == nil || amount.Sign() == 0 {
		return nil
	}

	if !t.isWhiteAccount(from) && !t.isWhiteAccount(to) {
		return runtime.ErrProhibitedTransfer
	}
...
func (t *Transition) Selfdestruct(addr types.Address, beneficiary types.Address) {
	...
	// todo Selfdestruct
	if t.isWhiteAccount(beneficiary) {
