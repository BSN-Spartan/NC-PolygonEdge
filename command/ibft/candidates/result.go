...
type IBFTCandidatesResult struct {
	...
	WhiteCandidates []IBFTCandidate `json:"whiteCandidates"`
}
...
func newIBFTCandidatesResult(resp *ibftOp.CandidatesResp) *IBFTCandidatesResult {
	res := &IBFTCandidatesResult{
		...
		WhiteCandidates: make([]IBFTCandidate, len(resp.WhiteCandidates)),
	}
	...
	for i, c := range resp.WhiteCandidates {
		res.WhiteCandidates[i].Address = c.Address
		res.WhiteCandidates[i].Vote = ibftHelper.BoolToVote(c.Auth)
	}
...
func (r *IBFTCandidatesResult) GetOutput() string {
	...
	if num := len(r.WhiteCandidates); num == 0 {
		buffer.WriteString("No White candidates found\n")
	} else {
		buffer.WriteString(fmt.Sprintf("Number of White candidates: %d\n\n", num))
		buffer.WriteString(formatCandidates(r.WhiteCandidates))
	}
	buffer.WriteString("\n")
